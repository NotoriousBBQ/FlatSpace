    ...
    public class GameAI : MonoBehaviour
    {
        ...
        private void ShipGrotsits(ScoreMatrix scoreMatrix, int playerID, List<GameAIOrder> orders, List<Planet.PlanetUpdateResult> surplusResults)
        {
            ...
            foreach (var actionNTuple in actionList)
            {
                ...
                var originPlanet = GetPlanet(actionNTuple.Origin);
                var playerPopFraction = originPlanet.GetPopulationFraction(playerID);
                if (playerPopFraction > 0.0f)
        ...
        private bool IsValidColonizationTarget(Planet planet, int playerID)
        {
            if (planet.Population.Count == 0)
                return true;
            if (planet.PopulationTransferInProgress)
                return false;
            if (planet.Population.Count >= planet.MaxPopulation)
                return false;
            if (planet.PlayerWithMostPopulation() != playerID)
                return true;
            
            return false;
        }

        private void ProcessColonizers(List<Planet.PlanetUpdateResult> results, int playerID, List<GameAIOrder> orders)
        {
            ...
            var possibleColonizerTargets = GameAIMap.PlanetList.FindAll(
                x => IsValidColonizationTarget(x, playerID));
        ...
        private void SendColonizers(ScoreMatrix scoreMatrix, List<GameAIOrder> orders, int playerID, List<Planet.PlanetUpdateResult> possibleColonizers)
        {
            ...
            foreach (var actionNTuple in actionList)
            {
                var changeAmount = Convert.ToInt32(possibleColonizers.Find(x => x.Name == actionNTuple.Origin).Data);
